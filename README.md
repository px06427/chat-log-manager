...
> 🎯 **特色**：一键跳转到历史消息原文位置，批量导入/导出，全局内容搜索，自动跟随主题，移动端适配。
...
- 🎯 **深海锚点引擎**: 点击搜索结果直接“跃迁”。支持指定楼层跳转、跨角色自动切换、自动加载文件，并精准定位到消息原文位置，辅以高亮反馈。
- 📦 **极速批量操作**: 支持一键勾选多个会话，进行批量删除（含安全确认，自动避开正在使用的会话）或批量导出。
- 📄 **多格式导出**: 完美支持将聊天记录导出为 `Markdown`、`JSONL (酒馆原生格式)`、`TXT` 格式，支持单消息/指定消息区间导出。
...
*   **数据安全**：执行批量删除操作前，请务必确认已做好重要记录的备份。
*   **浏览器限制**：在处理超大规模数据（超过 10 万条）时，索引构建可能需要数秒时间，请耐心等待进度条完成。
*   **显示限制**：为了最佳体验，建议在酒馆原生设置中开启 `Chat Display Limit`。
...
*   **Scrolling**: @tanstack/vue-virtual
